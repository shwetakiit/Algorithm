...
public class MishaAndCandies {
	...
	public static void noOfCandyEatenbyMisha(int arr[], int B) {
		int noOfEatenCandy = 0;
		...
		while (!queue.isEmpty()) {
			...
			int boxWithMinCandy = queue.poll();
			if (boxWithMinCandy > B) {
				continue;
			}
			int eatenCany_floor = boxWithMinCandy / 2;
			int remaingCandy = boxWithMinCandy - eatenCany_floor;

			if (!queue.isEmpty()) {
				int secondBoxWithMincady = queue.poll();
				queue.add(secondBoxWithMincady + remaingCandy);
			}

			noOfEatenCandy = noOfEatenCandy + eatenCany_floor;

		}
		System.out.println("No of eaten candy=" + noOfEatenCandy);
